...
class BotState:
    ...
    # Starts at 0 — bot will NOT quote until this is parsed from news.
...
class QuotingParams:
    ...
    # --- Correlation hedge ---
    # Bias quoting to organically build hedge positions via correlated tickers.
    # If SPNG is long 2000 and SMMR is corr=+0.8, bias SMMR quotes to prefer
    # getting short (tighten ask, widen bid). No new orders — just skew adjustment.
    corr_hedge_enabled: bool = True
    corr_hedge_skew_per_share: float = 0.0003  # skew per share of correlated exposure
    corr_hedge_max_skew: float = 0.03          # cap on correlation hedge skew ($)
    corr_hedge_min_corr: float = 0.40          # ignore correlations weaker than this

    # --- Net limit awareness ---
    # Net position = signed sum of all positions (can be negative).
    # Fined per share over net_limit in REAL-TIME.
    net_limit_warning_pct: float = 0.60      # start skewing at 60% of net limit
    net_limit_hard_pct: float = 0.85         # block increasing side at 85% of net limit
    net_skew_per_share: float = 0.0003       # extra skew per share of net position
...
def compute_quotes(
    ...
    num_tickers: int = 4,       # number of tickers quoting simultaneously
    corr_hedge_skew: float = 0.0,  # extra skew from correlation hedge (positive = want to sell)
    ...
    # --- Net limit HARD guardrails (size caps) ---
    # Cap increasing-side size by net room (like aggregate room cap).
    # Price-blocking happens later after bid_px_out/ask_px_out are set.
    net_ratio = abs(net_position) / max(net_limit, 1)
    if net_ratio >= 1.0:
        # Over net limit: zero out increasing side size
        if net_position > 0:
            bid_size = 0
        else:
            ask_size = 0
    elif net_ratio >= params.net_limit_hard_pct:
        if net_position > 0:
            bid_size = 0
        else:
            ask_size = 0

    # Also cap increasing-side size by net room
    if net_limit > 0:
        net_room = max(0, net_limit - abs(net_position))
        per_ticker_net_room = net_room // max(1, num_tickers)
        if net_position >= 0:
            bid_size = min(bid_size, max(0, per_ticker_net_room))
        else:
            ask_size = min(ask_size, max(0, per_ticker_net_room))

    # --- Correlation hedge skew ---
    # Biases quotes to build offsetting positions in correlated tickers.
    # Positive corr_hedge_skew = portfolio wants us to sell this ticker (shifts quotes down).
    # Still capped at 50% of half-spread to maintain edge on both sides.
    if abs(corr_hedge_skew) > 0.0001:
        capped_corr = max(-max_allowed_skew, min(max_allowed_skew, corr_hedge_skew))
        skew += capped_corr
    ...
    # Cap order sizes so we can't overshoot the aggregate limit even if filled.
    # Room = how many more shares of aggregate we can absorb.
    # CRITICAL: divide by num_tickers because ALL tickers' orders can fill in the
    # same tick (especially with 35-40 competing bots). Each ticker only gets its
    # fair share of the remaining room.
    agg_room = max(0, agg_limit - int(agg_position_ratio * agg_limit))
    per_ticker_room = agg_room // max(1, num_tickers)
    if position >= 0:
        # Long: buys increase aggregate, sells decrease (or keep same if reducing)
        bid_size = min(bid_size, max(0, per_ticker_room))
    else:
        # Short: sells increase aggregate, buys decrease (reducing)
        ask_size = min(ask_size, max(0, per_ticker_room))
    ...
    # --- Net limit price-blocking (must be after bid_px_out/ask_px_out are set) ---
    if net_ratio >= 1.0:
        if net_position > 0:
            bid_px_out = None; bid_size = 0
        else:
            ask_px_out = None; ask_size = 0
    elif net_ratio >= params.net_limit_hard_pct:
        if net_position > 0:
            bid_px_out = None; bid_size = 0
        else:
...
def get_worst_case_aggregate(state: BotState) -> int:
    """
    Worst-case aggregate position if ALL working orders fill.
    For each ticker, assume both bid and ask fill → position moves by those quantities.
    The worst case for aggregate is whichever direction increases |position| more.
    """
    total = 0
    for ts in state.per_ticker.values():
        pos = ts.position
        wr = ts.working
        # Working bid (BUY) would increase position, ask (SELL) would decrease
        worst_long = pos + (wr.bid_qty or 0)   # if bid fills
        worst_short = pos - (wr.ask_qty or 0)   # if ask fills
        # Worst case for aggregate is whichever has higher abs
        total += max(abs(worst_long), abs(worst_short))
    return total


def get_limits_info(api: RITClient) -> Tuple[int, int, float, int, int]:
    """
    Returns (gross_position, gross_limit, gross_fine, net_limit, api_net).
    Falls back to safe defaults if the API call fails.
    """
    limits = api.get_limits()
    if not limits:
        return 0, 25000, 5.0, 25000, 0  # safe defaults
    ...
    api_net = int(lim.get("net", 0))  # current net position from API
    return gross, gross_limit, gross_fine, net_limit, api_net
...
def compute_corr_hedge_skews(
    state: BotState,
    corr: CorrelationTracker,
    params: QuotingParams,
) -> Dict[str, float]:
    """
    Compute a per-ticker skew that biases quoting to build hedge positions.

    For each ticker, look at all OTHER tickers' positions and their correlation.
    If SPNG is long 2000 and SMMR has corr=+0.8 with SPNG, then the portfolio
    wants SMMR to get SHORT to hedge. So we add positive skew to SMMR
    (shifting both quotes down → bid lower, ask lower → more likely to sell).

    This is the OPPOSITE of the old cross-hedge: no new orders, no market orders,
    no aggregate increase. Just a gentle bias on normal market-making quotes.
    """
    tickers = list(state.per_ticker.keys())
    skews: Dict[str, float] = {t: 0.0 for t in tickers}

    if not params.corr_hedge_enabled:
        return skews

    if not corr.ready(min_points=20):
        return skews

    mat = corr.corr_matrix()

    for target in tickers:
        # For this ticker, compute how much correlated exposure the rest of
        # the portfolio has. If other tickers are long and positively correlated,
        # we want to sell this ticker (positive skew).
        weighted_exposure = 0.0
        for other in tickers:
            if other == target:
                continue
            c = mat.get((target, other), 0.0)
            if abs(c) < params.corr_hedge_min_corr:
                continue  # correlation too weak to act on
            # other's position × correlation = how much "virtual" position
            # this ticker is carrying via the correlated asset
            weighted_exposure += state.s(other).position * c

        # weighted_exposure > 0 means portfolio is effectively long this ticker
        # via correlated assets → we want to sell this ticker → positive skew
        raw = weighted_exposure * params.corr_hedge_skew_per_share
        skews[target] = max(-params.corr_hedge_max_skew,
                            min(params.corr_hedge_max_skew, raw))
...
def run_market_maker(
    ...
    # Correlation hedge state
    last_hedge_ts: float = 0.0
    corr_skews: Dict[str, float] = {t: 0.0 for t in tickers}
    ...
    # ---- WAIT FOR AGGREGATE LIMIT FROM NEWS ----
    # Don't trade until we know the real limit. Poll news aggressively on startup.
    print(f"[BOT] Waiting for aggregate position limit from news...")
    while not state.aggregate_limit_parsed:
        case = api.get_case()
        if case and case.get("status") == "ACTIVE":
            news.poll(api)
            if news.parsed_aggregate_limit is not None:
                state.aggregate_limit = news.parsed_aggregate_limit
                state.aggregate_limit_parsed = True
                print(f"[BOT] *** Aggregate limit = {state.aggregate_limit:,} (from news) ***")
                break
        time.sleep(0.25)
    print(f"[BOT] Limit confirmed. Starting market-making.")
    ...
    while True:
        ...
        gross, gross_limit, gross_fine, net_limit, api_net = get_limits_info(api)
        agg_pos = get_aggregate_position(state)
        net_position = sum(ts.position for ts in state.per_ticker.values())  # signed sum
        # Use the worse of local vs API net (API is source of truth for fines)
        if abs(api_net) > abs(net_position):
            net_position = api_net

        # Compute ticks-to-close early (needed by limit system + guard + flatten)
        ticks_to_close = compute_ticks_until_close(current_tick, params.ticks_per_day)

        # Update aggregate limit from news (may change mid-heat if new news drops)
        if news.parsed_aggregate_limit is not None:
            if news.parsed_aggregate_limit != state.aggregate_limit:
                old = state.aggregate_limit
                state.aggregate_limit = news.parsed_aggregate_limit
                state.aggregate_limit_parsed = True
                print(f"[BOT] Aggregate limit updated: {old:,} -> {state.aggregate_limit:,} (from news)")

        # TWO-TIER LIMIT SYSTEM:
        # - Aggregate limit (from news): $10/share fine assessed at each MARKET CLOSE
        # - Gross limit (from API):      $5/share fine assessed in REAL-TIME
        # Mid-day: we can safely use up to gross_limit (the higher one).
        # Near close: tighten to aggregate_limit so we're under it when assessed.
        agg_limit_news = state.aggregate_limit  # strict limit, only matters at close
        agg_limit_gross = gross_limit if gross_limit > 0 else 25000  # lenient, real-time

        # Near close (within flatten window), use the stricter aggregate limit.
        # Otherwise, use gross_limit to allow bigger positions mid-day.
        if ticks_to_close <= params.flatten_start_ticks_before_close:
            agg_limit = min(agg_limit_news, agg_limit_gross)  # strict near close
        else:
            agg_limit = agg_limit_gross  # more room mid-day

        if loop_count % 500 == 0:
            print(f"[BOT] Limits: agg(news)={agg_limit_news}, gross={agg_limit_gross}, "
                  f"effective={agg_limit}, ticks_to_close={ticks_to_close}")

        # Use the WORSE of: local agg_pos vs API gross position.
        # Local state can be stale if fills happened between sync and now.
        effective_agg = max(agg_pos, gross)
        agg_ratio = effective_agg / max(agg_limit, 1)

        # ---- POSITION GUARD: If aggregate position exceeds limit ----
        if effective_agg > agg_limit:
            ...
            sync_positions(api, state)
            agg_pos = get_aggregate_position(state)
            effective_agg = max(agg_pos, gross)  # re-check with API gross
            agg_ratio = effective_agg / max(agg_limit, 1)

        # ---- POSITION GUARD: If NET position exceeds limit ----
        net_ratio = abs(net_position) / max(net_limit, 1)
        if net_ratio >= 1.0:
            # OVER NET LIMIT: cancel ALL orders, post reducing orders
            api.cancel_all()
            for ts_ks in state.per_ticker.values():
                ts_ks.working = OrderRef()

            print(f"  [NET-GUARD] net={net_position:+d}/{net_limit} ({net_ratio:.0%}) — OVER LIMIT")

            # Need to reduce net: if net > 0, sell the biggest longs.
            # If net < 0, buy the biggest shorts.
            if net_position > 0:
                # Sort tickers by position descending (biggest longs first)
                sorted_ts = sorted(
                    state.per_ticker.values(),
                    key=lambda ts_sort: ts_sort.position,
                    reverse=True,
                )
                excess = abs(net_position) - net_limit
                for ts_reduce in sorted_ts:
                    if excess <= 0 or ts_reduce.position <= 0:
                        break
                    reduce_qty = min(ts_reduce.position, excess, params.flatten_market_order_max)
                    mid_px = ts_reduce.last_mid or 25.0
                    spr = ts_reduce.last_spread or 0.10
                    px = round(mid_px + spr * 0.05, 2)  # passive ask
                    result = api.post_order(
                        ticker=ts_reduce.ticker, side="SELL", qty=reduce_qty,
                        order_type="LIMIT", price=px, wait=False,
                    )
                    if result:
                        excess -= reduce_qty
                        print(f"    [NET-REDUCE] SELL {reduce_qty} {ts_reduce.ticker} @ {px:.2f}")
            else:
                # Sort tickers by position ascending (biggest shorts first)
                sorted_ts = sorted(
                    state.per_ticker.values(),
                    key=lambda ts_sort: ts_sort.position,
                )
                ...
                for ts_reduce in sorted_ts:
                    ...
                    mid_px = ts_reduce.last_mid or 25.0
                    spr = ts_reduce.last_spread or 0.10
                    px = round(mid_px - spr * 0.05, 2)  # passive bid
                    result = api.post_order(
                        ticker=ts_reduce.ticker, side="BUY", qty=reduce_qty,
                        order_type="LIMIT", price=px, wait=False,
                    )
                    if result:
                        excess -= reduce_qty
                        print(f"    [NET-REDUCE] BUY {reduce_qty} {ts_reduce.ticker} @ {px:.2f}")

            sync_positions(api, state)
            net_position = sum(ts.position for ts in state.per_ticker.values())
        ...
        # PRE-CLOSE DARK: In the last 2 ticks before close, cancel EVERYTHING.
        # Fines are assessed at market close — any working order that fills in the
        # last tick can push us over the limit with no time to correct.
        if ticks_to_close <= 2 and ticks_to_close > 0:
            api.cancel_all()
            for ts_dark in state.per_ticker.values():
                ts_dark.working = OrderRef()
            if current_tick != last_tracker_tick:
                print(f"  [PRE-CLOSE DARK] {ticks_to_close} ticks to close — all orders cancelled, no quoting")
        ...
        # ---- 5b. Correlation hedge skews (every 2 seconds, only when not flattening) ----
        now = time.time()
        if not is_flattening and (now - last_hedge_ts) >= 2.0:
            corr_skews = compute_corr_hedge_skews(state, corr, params)
            last_hedge_ts = now
        ...
        effective_agg = max(agg_pos, gross)
        agg_ratio = effective_agg / max(agg_limit, 1)
        agg_over = effective_agg > agg_limit
        ...
        for t in tickers_to_refresh:
            ...
            if is_flattening and ticks_to_close > params.flatten_hard_ticks_before_close:
                ...
                bid_px, ask_px, bid_sz, ask_sz = compute_quotes(
                    ...
                    corr_hedge_skew=corr_skews.get(t, 0.0),
                )
        ...
        if (now - last_diag_print) > 3.0:
            ...
            for t in tickers:
                ts = state.s(t)
                wr = ts.working
                cs = corr_skews.get(t, 0.0)
                cs_str = f" corr_skew={cs:+.4f}" if abs(cs) > 0.0001 else ""
                print(
                    f"    {t}: bid={wr.bid_px or 0:.2f}x{wr.bid_qty}  "
                    f"ask={wr.ask_px or 0:.2f}x{wr.ask_qty}  "
                    f"fills={ts.num_fills} posts={ts.num_posts}{cs_str}"
                )
...
if __name__ == "__main__":
    ...
    params = QuotingParams(
        ...
        order_size=4000,              # 4000 shares per side — bigger = more rebates per fill
        ...
        flatten_market_order_max=10000,       # max shares per flatten order (matches bigger positions)
        # --- Per-ticker rebate sizing ---
        ticker_rebates={"SPNG": 0.03, "SMMR": 0.04, "ATMN": 0.035, "WNTR": 0.045},
        rebate_sizing_enabled=True,           # ENABLED — scale size by rebate (WNTR gets biggest)
        rebate_baseline=0.03,                 # SPNG is baseline (1x), WNTR = 1.5x
        ...
        # --- News blackout window ---
        news_blackout_ticks=7,                # don't quote for first 7 ticks of each day
        # --- Net limit awareness ---
        net_limit_warning_pct=0.50,           # start net-skew at 50% of net limit
        net_limit_hard_pct=0.85,              # block increasing side at 85% of net limit
        net_skew_per_share=0.0003,
        # --- Correlation hedge ---
        corr_hedge_enabled=True,              # bias quoting to build hedge positions
        corr_hedge_skew_per_share=0.0003,     # skew per share of correlated exposure
        corr_hedge_max_skew=0.03,             # max $0.03 hedge skew
        corr_hedge_min_corr=0.40,             # ignore correlations below 0.4
        # --- Per-ticker position cap ---
        per_ticker_limit_fraction=0.28,       # each ticker max 28% of effective limit (aggregate room cap prevents overshoot)
        # --- Fill-rate auto-tuning ---
