# Added: "New added sleep" set to 0.5
# Changed: case.tick % 60 <= 1 from % 60 == 59 "Safety Period"
...
class Algorithm:
    ...
    def make_market(self, ticker: str) -> None:
        ...
        while True:
            ...
            if case.status == Case.Status.ACTIVE:
                # Safety Periods
                if case.tick % 60 == 58 or case.tick % 60 <= 1: # CHANGED from % 60 == 59
                    self.call_safely(self.close, ticker)
                    sleep(2) # CHANGED from 2.5
                
                elif abs(security.position) > 7500:
                    self.call_safely(self.close, ticker)
                    sleep(0.1)
                    ...
                    security = self.rit.get_securities(ticker=ticker)[0]
                    bid_quantity = 7500 - security.position
                    ask_quantity = 7500 + security.position
                    book = self.rit.get_securities_book(ticker=ticker)

                                    
                    if abs(security.position) > 7500:
                        self.call_safely(self.close, ticker)
                        sleep(0.1)
                    ...
                    # sleep(0.5) # New added sleep, most profitable at 0.5
    def run(self) -> None:
        processes = []
...
def main() -> None:
    rit = RIT(x_api_key=X_API_KEY,API_ENDPOINT=API_ENDPOINT,AUTHORIZATION=AUTHORIZATION)
