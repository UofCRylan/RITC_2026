"""
RITC 2026 — Liquidity Risk Case: Tender Tool v7
=================================================
Requirements:  pip install requests matplotlib
Usage:         python rit_tender_tool_v7.py
Controls:      Ctrl+"+"/"-" zoom
"""

# Add a section that shows the liquidity
# fix the cost so that it is per stock
# vwap should be historic price of market not position
...
# ─── CONFIG ───────────────────────────────────────────────────────────────
RIT_API_BASE = "http://localhost:9999/v1"
API_KEY = "62BARBC1"
POLL_MS = 250
...
class RIT:
    ...
    def security(self, tk):    return self._get("/securities", {"ticker": tk})
    ...
    def get_position(self, ticker):
        """Fetch live position for a single ticker. Returns int or None."""
        secs = self.security(ticker)
        if secs and isinstance(secs, list) and len(secs) > 0:
            return int(secs[0].get("position", 0))
        return None

    def market_order(self, ticker, qty, action):
        """Send a single market order (no chunking). Returns result dict."""
        mx = MAX_ORDER.get(ticker, DEFAULT_MAX)
        q = min(abs(int(qty)), mx)
        return self._post("/orders", {
            "ticker": ticker, "type": "MARKET",
            "quantity": q, "action": action})

    def safe_exit(self, ticker, target_qty, action):
        """
        Exit a position in chunks, re-checking live position between
        each chunk to prevent overshooting into the opposite side.

        target_qty: total shares to exit (positive int)
        action: "BUY" or "SELL"
        ...
        while remaining > 0:
            # Re-check current position before each chunk
            live_pos = self.get_position(ticker)
            if live_pos is not None:
                abs_live = abs(live_pos)
                # If position is already flat or flipped, STOP
                if abs_live == 0:
                    break
                # If position flipped sides, STOP (we overshot somehow)
                if action == "SELL" and live_pos < 0:
                    break
                if action == "BUY" and live_pos > 0:
                    break
                # Only send what's actually still open
                remaining = min(remaining, abs_live)

            chunk = min(remaining, mx)
            if chunk <= 0:
                break

            r = self.market_order(ticker, chunk, action)
            results.append(r)
            remaining -= chunk

            # Small delay between chunks to let fills settle
            if remaining > 0:
                time.sleep(0.05)
...
class SecurityPanel(tk.Frame):
    def __init__(self, parent, ticker, rit, **kw):
        ...
        self._exiting = False  # guard against double-fire
        self._build()

    def _build(self):
        # ── Header row 1: ticker, price, position ──
        ...
        # ── Header row 2: Cost, VWAP ──
        hdr2 = tk.Frame(self, bg=BG_PANEL)
        hdr2.pack(fill="x", padx=10, pady=(1, 4))

        self.lbl_cost = tk.Label(hdr2, text="Cost: —", font=(MONO, 9),
                                  fg=FG_DIM, bg=BG_PANEL)
        self.lbl_cost.pack(side="left")

        self.lbl_vwap = tk.Label(hdr2, text="VWAP: —", font=(MONO, 9),
                                  fg=FG_DIM, bg=BG_PANEL)
        self.lbl_vwap.pack(side="left", padx=(16, 0))
        ...
        # Row 5: WTA price entry frame (children shown/hidden)
        ...
        # Row 7: REJECT — fixed position, always below accept
        ...
        # ── Trade row (50/50 grid, NEVER auto-updates qty) ──
    ...
    def update_market(self, price, bid, ask, position, vwap=0.0, cost=0.0):
        ...
        # Cost and VWAP
        if position != 0 and vwap > 0:
            self.lbl_vwap.config(text=f"VWAP: ${vwap:.2f}", fg=FG)
            # Cost = total cost basis = vwap * |position|
            total_cost = vwap * abs(position)
            self.lbl_cost.config(text=f"Cost: ${total_cost:,.0f}", fg=FG)
        else:
            self.lbl_vwap.config(text="VWAP: —", fg=FG_DIM)
            self.lbl_cost.config(text="Cost: —", fg=FG_DIM)

        # Trade button direction (NEVER touch qty_entry)
        if position > 0:
        ...
        # Only enable exit if we have a position AND not currently exiting
        if position != 0 and not self._exiting:
            self.btn_exit.config(state="normal")
        elif position == 0:
            self._exiting = False  # reset guard when flat
    ...
    def update_tender(self, tender, current_tick, tender_count=1):
        ...
        # ALWAYS enable accept and reject when tender active
    ...
    def _manual_trade(self):
        ...
        def do():
            # For manual trades, also use safe_exit to prevent overshoot
            self.rit.safe_exit(self.ticker, qty, action)
    ...
    def _exit_position(self):
        pos = self.position
        if pos == 0 or self._exiting:
            return

        self._exiting = True  # guard against double-click
        ...
        def do():
            ...
            def ui():
                self._exiting = False
                self.btn_exit.config(text="EXIT POSITION")
                # State will be re-enabled by update_market when position updates
            self.after(0, ui)
        ...
        def fetch():
            try:
                ...
                for sec in secs:
                    ...
                    vwap = sec.get("vwap", 0) or 0
                    # Cost per share = VWAP of the current position
                    cost = vwap

                    self.root.after(0, lambda _t=t, _p=price, _b=bid, _a=ask,
                                    _pos=pos, _vwap=vwap, _cost=cost:
                                    self.panels[_t].update_market(
                                        float(_p) if _p else None,
                                        float(_b) if _b else None,
                                        float(_a) if _a else None,
                                        _pos,
                                        float(_vwap) if _vwap else 0.0,
                                        float(_cost) if _cost else 0.0))
